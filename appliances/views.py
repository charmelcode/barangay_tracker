...
from .models import Category, BorrowRequest
...
def return_appliance(request, request_id):
    ...
    if request.method == 'POST':
        with transaction.atomic():
            # Increase the appliance quantity
            appliance = borrow_request.appliance
            appliance.quantity += borrow_request.quantity
            appliance.save()
            ...
            messages.success(request, f"You have successfully returned {borrow_request.quantity} {appliance.name}(s).")
